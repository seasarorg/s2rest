...
public class Jsr311ComponentRegisterImpl implements Jsr311ComponentRegister {
	private static final Logger log = Logger
			.getLogger(Jsr311ComponentRegisterImpl.class);
	...
	private void attachToRouter(Map<String, SeasarResourceFinder> uriMapping,
		...
		for (Iterator<String> iter = uriMapping.keySet().iterator(); iter
				.hasNext();) {
	...
	private void introduce(ComponentDef componentDef,
			Map<String, SeasarResourceFinder> uriMapping, String classUriPattern) {
		...
		if (methods.isEmpty()) {
			...
			throw new UnsupportedOperationException(
					"stateful resource is not supported since JSR311 version 0.6");
	...
	private void inspectEachHttpMethod(ComponentDef componentDef,
			Map<String, SeasarResourceFinder> uriMapping,
			String classUriPattern, boolean isStateful,
			Class<? extends Annotation> httpAnnotationClass) {

		List<Method> methods = AnnotationUtils.getAnnotatedMethods(componentDef
				.getComponentClass(), httpAnnotationClass);
		for (Method method : methods) {
			inspectHttpMethod(method, componentDef, uriMapping,
					classUriPattern, isStateful);
		}

	}

	private void inspectHttpMethod(Method method, ComponentDef componentDef,
			Map<String, SeasarResourceFinder> uriMapping,
			String classUriPattern, boolean isStateful) {
		...
		if (methodUriTemplate != null) {
			...
			if (StringUtil.isBlank(classUriPattern)) {
				...
				throw new IllegalStateException("no Path available for "
						+ ClassUtil.getShortClassName(componentDef
								.getComponentClass()) + "." + method.getName());
			}
			uriPattern = classUriPattern;
		}

		SeasarResourceFinder finder = getFinderFrom(uriMapping, uriPattern,
				componentDef);
		...
		log.info("attach ["
				+ ClassUtil.getShortClassName(componentDef.getComponentClass())
				+ "." + method.getName() + "] to " + httpMethodStr + ":["
				+ uriPattern + "]");
	}

	private void methodLevelProduceMime(Method method,
	...
	private String getHttpMethodStringFrom(Method method) {
		...
		throw new IllegalStateException(
				"Http annotation is not specified. If not specified, the name of the annotated method must begin with one of the method constants(in lowercase). method:["
						+ ClassUtil.getShortClassName(method
								.getDeclaringClass())
						+ "."
						+ method.getName()
						+ "]");
	...
	private SeasarResourceFinder getFinderFrom(
			Map<String, SeasarResourceFinder> uriMapping, String uriPattern,
			ComponentDef componentDef) {
		if (!uriMapping.containsKey(uriPattern)) {
			Router router = this.routerRegistry.getRouter();
			SeasarResourceFinder finder = new SeasarResourceFinder(router
					.getContext(), componentDef);
